...
export default function Founder() {
  ...
  useEffect(() => {
    ...
    const rawAvatar = (acf.img_link_data_avatar as string) || (raw as { Avatar?: string }).Avatar || "";
    const normalizedAvatar = normalizeImageUrl(rawAvatar);
    const avatar = normalizedAvatar !== "/placeholder.svg" ? normalizedAvatar : "";
    
    const mapped: Instructor = {
      title: raw?.title?.rendered || "",
      status: (acf.input_text_status as string) || (raw as { Status?: string }).Status || "Засновниця BFB",
      avatar,
      experience: (acf.input_text_experience as string) || (raw as { Experience?: string }).Experience || "",
      countTraining: (acf.input_text_count_training as string) || (raw as { Count_training?: string }).Count_training || "",
      certificates: (acf.input_text_certificates as string) || (raw as { Certificates?: string }).Certificates || "",
      linkInstagram: (acf.instagram as { url?: string })?.url || (raw as { Link_instagram?: string }).Link_instagram || "",
      textInstagram: (acf.instagram as { title?: string })?.title || (raw as { Text_instagram?: string }).Text_instagram || "",
      aboutMe: (acf.textarea_about_me as string) || (acf.textarea_description as string) || (raw as { About_me?: string; Description?: string }).About_me || (raw as { About_me?: string; Description?: string }).Description || "",
      myMission: (acf.textarea_my_mission as string) || (raw as { My_mission?: string }).My_mission || "",
    };
    setData(mapped);
  }, [raw]);
